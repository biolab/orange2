...
mkdir -p /mnt/debian/dists/lenny/main/binary-i386/
mkdir -p /mnt/debian/dists/lenny/main/source/

echo "Copying to repository new packages."
cd ..
rm -rf python-orange-svn-0.0.$DAILY_REVISION/
mv *$DAILY_REVISION*.deb /mnt/debian/dists/lenny/main/binary-i386/
...
perl -e '
for (<lenny/main/binary-i386/*.deb>) {
	m!.*/(.*?)_!;
	$fs{$1}++;
}
while (($f,$n) = each(%fs)) {
	next if $n <= 5;
	unlink for grep {-M > 30} <lenny/main/binary-i386/$f_*.deb>;
	unlink for grep {-M > 30} <lenny/main/source/$f_*>;
}
'

echo "Making packages list."
dpkg-scanpackages --multiversion lenny/main/binary-i386 /dev/null dists/ | gzip - > lenny/main/binary-i386/Packages.gz
dpkg-scansources lenny/main/source /dev/null dists/ | gzip - > lenny/main/source/Sources.gz

echo "Setting permissions."
chmod 644 lenny/main/binary-i386/* lenny/main/source/*
