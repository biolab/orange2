...
class FSSWrapperLearner_Class:
  def __init__(self, **kwds):
    self.__dict__.update(kwds)
    self.__dict__.setdefault("statistics", orngStat.CA)
    self.__dict__.setdefault("statsign", 1)
    self.__dict__.setdefault("folds", 5)

  def __call__(self, data, weight):
    origdata = data
    indices = orange.MakeRandomIndicesCV(data, folds=self.folds)
    res = orngTest.testWithIndices([self.baseLearner], (data, weight), indices)
    startCA = self.statistics(res)[0]
    print startCA,
    startCAs = [self.statistics(x)[0] for x in orngStat.splitByIterations(res)]
    while len(data.domain.attributes):
        bestCA = None
        for attr in data.domain.attributes:
            data2 = orange.Preprocessor_ignore(data, attributes=[attr])
            res = orngTest.testWithIndices([self.baseLearner], (data2, weight), indices)
            cCA = self.statistics(res)[0]
            #print attr.name, cCA
            if not bestCA or (cmp(cCA, bestCA)==self.statsign):
                cCAs = [self.statistics(x)[0] for x in orngStat.splitByIterations(res)]
                if (cmp(cCA, startCA) == self.statsign) or statc.wilcoxont(startCAs, cCAs)[1]>0.10:
                    bestCA, bestCand, bestCCAs = cCA, attr, cCAs
        if bestCA<0:
