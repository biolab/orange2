...
def readXML(runType = None, filename = docdir+"/testresults.xml"):
...
def saveXML(filename = docdir + "/testresults.xml"):
...
def do_help():
    ...
    print "  -n t       run each script for t times (default: 5)"
    ...
    for dir in os.listdir("."):
        ...
        for name, node, isNewFile, oldtestnode in test_set:
            ...
            for t in ["crash", "error", "new", "changed", "random1", "random2"]:
                remname = "outputs/%s.%s.%s.txt" % (name, zfill(runNo, 5), t)
                if os.path.exists(remname):
                    os.remove(remname)
                
            os.spawnl(os.P_WAIT, sys.executable, "-c", "../xtest1.py", name, `iterations`, `runNo`, `int(isNewFile)`)
