...
class uninstall(Command):
    ...
    user_options = [('orangepath=', None, "Orange installation path"),
                    ...
                    ('prefix=', None, "Installation prefix - for package building")]
    ...
    def finalize_options(self):
        ...
        if self.orangepath is None:
            ...
            if self.orangepath is None:
                self.orangepath = os.path.join(self.root, "lib", PythonVer,
                                               "site-packages", "orange")
            if self.docpath is None:
                self.docpath = os.path.join(self.root, "share", "doc", "orange")
            if self.libpath is None:
                self.libpath = os.path.join(self.root, "lib")
        else:
            if self.docpath is None:
    ...
    def run(self):
        if self.systemUninstall is True:
            if os.geteuid() != 0:
                ...
                sys.exit(1)
            self.orangepath = os.path.join(self.root, sys.prefix, self.orangepath)
            sysBinFile = os.path.join(self.root, sys.prefix, "bin", "orange")
...
class install_data_wrap(install_data):
    ...
    def finalize_options(self):
        ...
        print "root:%s , pref:%s, INSTALL_dir:%s" % (self.root, sys.prefix, self.install_dir)
        if self.install_dir != os.path.join(self.root, sys.prefix[1:]):
...
class install_wrap(install):
    ...
    def run(self):
        ...
        if self.orangepath == None:
            ...
            sysBinFile = os.path.join(self.root, 'usr', "bin", "orange")
            print "creating binary at root = ", self.root, ":", sysBinFile, "->", binFile
            ...
            for currentLib in OrangeLibList:
                try:
                    print "%s -> %s" % (os.path.join(sys.prefix,
                                            self.OrangeInstallDir,currentLib),
                                        os.path.join(self.root, self.OrangeInstallLib[1:],"lib"+currentLib))
        ...
        if self.orangepath == None:
            # joining a path with one that begins with a '/' doesn't do what we want it to do. 
            # Therefore, the first char from prefix should be eliminated.
            pth = os.path.join(self.root, sys.prefix[1:], self.OrangeInstallDir,"..","orange.pth")
        else:
        ...
        if self.orangepath != None:
            ...
            print "Orange installation dir: "+self.root+self.OrangeInstallDir
