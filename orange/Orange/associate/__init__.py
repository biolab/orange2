...
for feature-based machine learning problems, which are at the primary focus
of Orange. We have thus adapted the original algorithm to be more efficient
for the latter type of data, and to induce the rules in which, for contrast
to Agrawal's rules, both sides don't only contain features
(like "bread, butter -> jam") but also their values
("bread = wheat, butter = yes -> jam = plum"). As a further variation, the
algorithm can be limited to search only for classification rules in which
the sole feature to appear on the right side of the rule is the class feature.
...
so, you need to use the other association rules inducer, described in the
chapter :ref:`non-sparse-examples`.

Since the usual representation of examples described above is rather unsuitable
for sparse examples, AssociationRulesSparseInducer can also use examples
represented a bit differently. Sparse examples have no fixed
features - the examples' domain is empty, there are neither ordinary nor class
features. All values assigned to example are given as meta-attributes.
All meta-attributes need, however, be `registered with the domain descriptor
<http://orange.biolab.si/doc/reference/Domain.htm#meta-attributes>`_.
If you have data of this kind, the most suitable format for it is the
`basket format <http://orange.biolab.si/doc/reference/fileformats.htm#basket>`_.

In both cases, the examples are first translated into an internal
AssociationRulesSparseInducer's internal format for sparse datasets. The
algorithm first dynamically builds all itemsets (sets of features) that have
...
.. class:: Orange.associate.AssociationRulesSparseInducer
    ...
    Tells the inducer to store the examples covered by each rule and
    those confirming it.
...
The maxItemSets attribute deserves some explanation. The algorithm's
...
There is no general rule for knowing the itemset in advance (generally, value
should be around 0.3, but this depends upon the number of different items, the
diversity of examples...) so it's very easy to set the limit too low. In this
case, the algorithm can induce hundreds of thousands of itemsets until it
runs out of memory. To prevent this, it will stop inducing itemsets and
report an error when the prescribed maximum maxItemSets is exceeded.
In this case, you should increase the required support. On the other hand,
you can (reasonably) increase the maxItemSets to as high as you computer is
able to handle.
...
If examples are weighted, weight can be passed as an additional argument to
call operator.

To get only a list of supported item sets, one should call the method
getItemsets. The result is a list whose elements are tuples with two elements.
The first is a tuple with indices of features in the item set. Sparse examples
are usually represented with meta-attributes, so this indices will be negative.
The second element is  a list of indices supporting the item set, that is,
containing all the items in the set. If storeExamples is False, the second
element is None. ::
...
since we set storeExamples to True. ::
...
This way of representing the itemsets is not very programmer-friendly, but
it is much more memory efficient than and faster to work with than using
objects like Variable and Example.
...
(AssociationRulesInducer) is optimized for non-sparse examples in the usual
Orange form. Each example is described by values of a fixed set of features.
Unknown values are ignored, while values of features are not (as opposite to
the above-described algorithm for sparse rules). In addition, the algorithm
can be directed to search only for classification rules, in which the only
feature on the right-hand side is the class Feature.
...
.. class:: Orange.associate.AssociationRulesInducer(float asupp, float aconf)
    ...
    If 1 (default is 0), the algorithm constructs classification rules instead
    of general association rules.
    
    .. attribute:: storeExamples
    
    Tells the inducer to store the examples covered by each rule and those
    confirming it
...
AssociationRulesInducer can also work with weighted examples; the ID of weight
feature should be passed as an additional argument in a call.
...
AssociationRules which is basically a list of instances of AssociationRule.

.. class:: Orange.associate.AssociationRules

    .. attribute:: left, right
    
        The left and the right side of the rule. Both are given as Example.
        In rules created by AssociationRulesSparseInducer from examples that
        ...
        however, be tested by isSpecial (see documentation on
        `Value <http://orange.biolab.si/doc/reference/Value.htm>`_).
    ...
    .. attribute:: examples, matchLeft, matchBoth
        ...
        If storeExamples was True during induction, examples contains a copy
        of the example table used to induce the rules. Attributes matchLeft
        and matchBoth are lists of integers, representing the indices of
...
matchLeft but not in matchBoth. The memory friendlier and the faster way
