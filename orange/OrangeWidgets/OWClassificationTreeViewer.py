...
""",
        ...
        self.connect(self.v, SIGNAL("returnPressed(QListViewItem *)"), self.treeSelectionChanged)
...
##                self.v.setColumnWidthMode(i, QListView.Maximum)
    ...
    def treeSelectionChanged(self, item):
        # send the data
        #for ex in self.nodeClassDict[item].examples:
        #    print ex        
        self.visComboWin.append(OWComboVis())
        self.visComboWin[-1].cdata(OrangeData(self.nodeClassDict[item].examples))
        self.visComboWin[-1].show()
