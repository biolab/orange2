...
class OWMDS(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None, name="Multi Dimensional Scaling"):
        ...
        OWGUI.spin(stopping, self, "maxIterations", box="Max. Number of Iterations", min=1, max=100)
        ...
        self.infoB=QLabel("Num. iterations", infoBox)
    ...
    def start(self):
        ...
        while not self.done and numIter<self.maxIterations:
            ...
            numIter+=1
            self.infoB.setText("Num. iterations: %i" % numIter)
            if self.ReDraw:
                self.graph.updateData()
            qApp.processEvents()
...
class MDSGraph(OWVisGraph):
    ...
    def setPoints(self):
        if self.ColorAttr==0:
            ...
            colors=OWGraphTools.ColorPaletteHSV(100)
        ...
        for i,(x,y) in enumerate(self.mds.X[:len(self.data)-1]):
            ...
            if self.ColorAttr==0:
                ...
                index=int(math.floor(float(self.data[i][self.attributes[self.ColorAttr-1]]-minVal)*100/(maxVal-minVal)))
