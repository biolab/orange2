...
class OWParallelCoordinates(OWWidget):
	...
	def setShownAttributeList(self, data):
		...
		shown, hidden = OWVisAttrSelection.selectAttributes(data, self.graph, self.attrContOrder, self.attrDiscOrder, self.projections)
	...
	def updateShownAttributeList(self):
		if self.data != None:
			if self.projections == None and (self.attrContOrder == "VizRank" or self.attrDiscOrder == "VizRank" or self.middleLabels == "VizRank"):
				name = QFileDialog.getOpenFileName( ".", "Interesting projections (*.proj)", self, "", "Open Projections")
				name = str(name)
				if name == "":
