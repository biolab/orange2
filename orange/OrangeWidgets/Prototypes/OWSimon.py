...
#        self.simulationData.adjustSize()
        ...
        curve = PolygonCurve(QPen(QColor(0,0,0)), QBrush(QColor("white")))
        self.cameraRay = self.graph.insertCurve(curve)
        self.robotTrace = graph.addCurve("", Qt.black, QColor(160, 160, 160), 5, style = QwtPlotCurve.Lines, symbol = QwtSymbol.Ellipse, xData = self.xTrace, yData = self.yTrace)
        self.robotKey = graph.addCurve("", Qt.black, Qt.black, 1, style = QwtPlotCurve.Lines, symbol = QwtSymbol.NoSymbol, xData = [], yData = [])
        curve = PolygonCurve(QPen(QColor("darkGreen")), QBrush(QColor("darkGreen")))
