...
class OWInteractionAnalysis(OWWidget):
    ...
    def updateGraph(self):
        ...
        self.graph.set_axis_labels(xBottom, attributes)
        self.graph.set_axis_labels(yLeft, [""] + attributes[::-1])
...
class OWPlotAttributeHistogram(OWWidget):
    ...
    def updateGraph(self):
        ...
        for (ind, (count, progressCountDict, attr)) in enumerate(attrs):
            ...
            if self.progressLines and count*8 > maxProjCount:
                ...
                for i in range(4):
                    ...
                    curr += c
                    self.graph.addCurve("", black, black, 2, Qt.SolidLine, NoSymbol, xData = [ind-0.5+0.5*eps,ind+0.5-0.5*eps], yData = [curr, curr], lineWidth = 3)
        ...
        self.graph.set_axis_labels(xBottom, attrNames)
        self.graph.setYLaxisTitle("Number of appearances in top projections")

        if self.colorAttributes:
            classVariableValues = getVariableValuesSorted(self.data.domain.classVar)
            classColors = ColorPaletteHSV(len(classVariableValues))
            self.graph.addCurve("<b>" + self.data.domain.classVar.name + ":</b>", QColor(0,0,0), QColor(0,0,0), 0, symbol = NoSymbol, enableLegend = 1)
            for i,val in enumerate(classVariableValues):
                self.graph.addCurve(val, classColors[i], classColors[i], 15, symbol = Rect, enableLegend = 1)
...
class OWPlotProjectionQuality(OWWidget):
    ...
    def updateGraph(self):
        ...
        if not self.showDistributions:
            ...
            self.graph.addCurve("", c, c, 1, Qt.SolidLine, NoSymbol, xData = xVals, yData = yVals, lineWidth = self.lineWidth)
            
            self.graph.set_axis_scale(yLeft, min(yVals), max(yVals))
            self.graph.set_axis_scale(xBottom, min(xVals), max(xVals))
            ...
            self.graph.addCurve("", c, c, 1, Qt.SolidLine, NoSymbol, xData = xs, yData = ys, lineWidth = self.lineWidth)
            self.graph.setAxisScale(yLeft, min(ys), max(ys))
            self.graph.setAxisScale(xBottom, min(xs), max(xs))
...
##        butt.setMaximumWidth(60)
    ...
    def updateGraph(self):
        ...
        self.graph.setAxisScale(yLeft, 0, len(self.graphMatrix[0]), len(self.graphMatrix[0])/5)
        self.graph.setAxisScale(xBottom, 0, 1, 0.2)
    ...
    def graphOnMouseMoved(self, e):
        y = int(self.graph.invTransform(yLeft, e.y()))
