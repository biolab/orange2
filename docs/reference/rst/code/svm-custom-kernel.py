...
l1 = SVMLearner()
l1.kernel_func = kernels.RBFKernelWrapper(EuclideanConstructor(iris), gamma=0.5)
...
l2 = SVMLearner()
l2.kernel_func = kernels.RBFKernelWrapper(HammingConstructor(iris), gamma=0.5)
...
l3.kernel_func = kernels.CompositeKernelWrapper(
    kernels.RBFKernelWrapper(EuclideanConstructor(iris), gamma=0.5),
    kernels.RBFKernelWrapper(HammingConstructor(iris), gamma=0.5), l=0.5)
