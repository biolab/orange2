...
if 1: ### Definitions of regular expressions
  ...
  listpluginmethodsdef=re.compile(r"LIST_PLUGIN_METHODS\((?P<listtypeid>\w*)\)")
...
def detectListPluginMethods(line, listplugins):
  found=listpluginmethodsdef.search(line)
  if found:
    name=found.group("listtypeid")
    if not name in listplugins:
      listplugins.append(name)
    return 1
...
def parseFiles():
  ...
  for parsedFile in filenames:
    ...
    for line in infile:
      ...
      detectListPluginMethods(line, listplugins)
  ...
  return functions, constants, classdefs, listplugins, aliases
...
def writeExterns(targetname):
  ...
  for type in listplugins:
    externsfile.write("extern PyObject *%s_plugin_methods;\n" % type)
    
  externsfile.close()


def writeListPluginsFile(targetname):
  pluginsfile=open("px/"+targetname+".new", "wt")
  newfiles.append(targetname)

  pluginsfile.write("/* This file was generated by pyxtract \n   Do not edit.*/\n\n")

  pluginsfile.write('#include "orange.hpp"\n\n')
  for type in listplugins:
    pluginsfile.write("PyObject *%s_plugin_methods=PyDict_New();\n" % type)

  pluginsfile.write('\n\nPyObject *pluginsModule()\n'
                    '{ \n'
                    '  PyObject *m= PyModule_New("list_plugin_methods");\n\n'
                    '  return      m\n')
  for name in listplugins:
    pluginsfile.write('           && !PyModule_AddObject(m, "%s", %s_plugin_methods)\n' % (name, name))

  pluginsfile.write('        ? m : NULL;\n}\n\n')
  pluginsfile.write('PYCONSTANTFUNC(list_plugin_methods, pluginsModule)\n')
  pluginsfile.close()
...
def make():
  ...
  writeListPluginsFile(pluginsname)
  #parseChanges()
  renewfiles(newfiles)
...
def readArguments(args):
  global filenames, verbose, recreate, externsname, initializationname, pluginsname, action, listname, xmlname
  filenames, verbose, recreate = [], 0, 0
  externsname, initializationname, pluginsname = "externs.px", "initialization.px", "mod_list_plugins.cpp"
  listname = xmlname = ""
  action = []
  i=0
  while(i<len(args)):
...
if len(action):  
  newfiles=[]
  functions, constants, classdefs, listplugins, aliases = parseFiles()
