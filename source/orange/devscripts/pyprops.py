...
propdef=re.compile(r'\s*(?P<ctype>\w+)\s+(?P<cname>\w+)\s*;\s*//P(?P<flags>\w+)?\s*(\([>+](?P<pname>\w+)\s*\))?(?P<pdesc>[^\r\n]*)')
compdef=re.compile(r'\s*(?P<ctype>\w+)\s+(?P<cname>\w+)\s*;\s*//C')

propwarndef=re.compile(r'\\\\P')
compwarndef=re.compile(r'\s*__component')

preprocessorsdef = re.compile('\s*DECLARE\s*\(\s*(?P<name>\w*)')
...
listdef = re.compile(r'\s*#define\s*(?P<name>\w+)\s*_?TOrangeVector<')
...
def detectBuiltInProperties(hppfile):
    ...
    ispreprocessors = hppfile=="preprocessors.hpp"
    ...
    for line in ff:
      ...
      if istdidt:
          ...
          if found:
                ...
                continue
      if ispreprocessors:
          found = preprocessorsdef.match(line)
          if found:
                storeClass(currentClass, hppfile)
                currentClass = ClassDefinition("TPreprocessor_"+found.group("name"), "TPreprocessor")
                continue
      if iscallback:
      ...
      if found:
        if not currentClass:
            ...
            ctype, cname, flags, pname, pdesc = found.group("ctype", "cname", "flags", "pname", "pdesc")
            normal = ctype in normalTypes
            if not normal and ctype[0]=="P":
              ctype = "T"+ctype[1:]
            currentClass.properties.append((ctype, cname, pname or cname, pdesc, flags and ("R" in flags) or 0, flags and ("O" in flags) or 0, normal))
        continue
